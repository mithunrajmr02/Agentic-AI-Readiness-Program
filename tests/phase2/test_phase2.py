...
def test_ingest_script_entrypoint():
    ...
    # Resolved from this file rather than the process CWD: a relative literal only
    # worked when pytest happened to be invoked from the repository root.
    ingest_py = os.path.join(
        os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
        "src", "rag", "ingest.py",
    )

    # Patched at the library boundary, not on src.rag.ingest: runpy executes the
    # file as a brand-new "__main__" module, so patches on the already-imported
    # src.rag.ingest module object would not apply to it. Without these, the
    # entrypoint drops and re-embeds the live chroma_db against the real Google
    # embeddings API -- a network call that mutates the store the retrieval tests
    # in this same file query.
    with patch.object(Chroma, "from_documents", return_value=MagicMock()), \
         patch.object(chromadb, "PersistentClient", return_value=MagicMock()):
