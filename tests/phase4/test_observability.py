...
def test_otel_span():
    ...
    # The duplicated patch pairs this block used to carry existed only to cover
    # both the canonical module and the phaseN re-export facade. The facades are
    # gone, so one patch per target is enough.
    with patch("src.mcp_server.mcp_app.tracer", test_tracer):
