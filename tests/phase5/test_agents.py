...
def test_demand_api_error():
    ...
    s = initial_state(999)
    with patch("src.agents.multi_agent.agents.requests.get", side_effect=Exception("down")), \
...
def test_reorder_recommends(state_after_forecast):
    ...
    with patch("src.agents.multi_agent.agents._llm") as ml:
        ...
        from src.agents.multi_agent.agents import reorder_agent
...
def test_supplier_quote(state_after_reorder):
    ...
    with patch("src.agents.multi_agent.agents.requests.get") as mg, patch("src.agents.multi_agent.agents._llm") as ml:
        ...
        from src.agents.multi_agent.agents import supplier_coordinator
...
def test_auditor_generates(state_after_reorder):
    ...
    with patch("src.agents.multi_agent.agents._llm") as ml:
        ...
        from src.agents.multi_agent.agents import inventory_auditor
